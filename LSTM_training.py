...
class Model(object):
    ...
    def __init__(self, model_path, train_mode=True, input_dim=25, T=180, prev=16,
       ...
       # self.opioidPK = tf.placeholder(tf.float32,shape=[None,5])   #k1,k12,k21,kout,opioid dose
       # self.naloxonePK = tf.placeholder(tf.float32,shape=[None,8]) #"F1","kin","ktr","kout2","k2","V1",naloxone dose, delay, threshold
        ...
        #xe = self.input_embedding(self.x[0:4,:])                         
...
#                pdb.set_trace()
    ...
    def retrain(self, train_set, valid_set, maxEpoch=10):
        
         with tf.Session() as sess:
            
            saver = tf.train.Saver()
            saver.restore(sess, "saved_model2/mymodel-72")
...
#                pdb.set_trace()
    ...
    def get_yprev_opioidPK(self, t):
        ...
        return yp_init_opioidPK if t == 0 else tf.concat([self.y_prev_opioidPK[:,1:], self.ys_opioidPK[t-1]], 1)
    ...
    def get_yprev_naloxonePK(self, t):
        ...
        return yp_init_naloxonePK if t == 0 else tf.concat([self.y_prev_naloxonePK[:,1:], self.ys_naloxonePK[t-1]], 1)
    

path = r'/scratch/lizhi/opioids/shuffled' # use your path
all_files = glob.glob(os.path.join(path , "shuffled_bs1"))
mpg = []
for filename in all_files:
    df = pd.read_csv(filename, index_col=None, header=0)
    mpg.append(df)

npg = np.concatenate(mpg)
results_train=np.array(npg).astype("float")

path = r'./bs2' # use your path
all_files = glob.glob(os.path.join(path , "car*bs2"))
mpg = []
for filename in all_files:
    df = pd.read_csv(filename, index_col=None, header=0)
    mpg.append(df)
...
npg = np.concatenate(mpg)
results_valid0=np.array(npg).astype("float")

SIZE = np.rint(np.size(results_valid0,0)*0.4).astype(int); # 40% of *bs2 is used for validation 
idx = np.random.choice(np.size(results_valid0,0),size=SIZE,replace=False); # Shuffling the bs2 data 
results_valid = results_valid0[idx,:]
...
if __name__ == "__main__":
    ...
    # with open('./bs1/carfentanil_1_120_0.4_bs1') as csvfile1:
    #     mpg = list(csv.reader(csvfile1, quoting=csv.QUOTE_NONNUMERIC))
    #     results_train=np.array(mpg).astype("float")
 
    # with open('./bs2/merged.bs2') as csvfile1:
    #     mpg = list(csv.reader(csvfile1, quoting=csv.QUOTE_NONNUMERIC))
    #     results_valid=np.array(mpg).astype("float")        
    ...
    #fixed the bugs below len(results)
    train_size=int(len(results_train))
    valid_size=int(len(results_valid))
    #note below 0:15 extracts 0-14 and 15:976 extracts 15-975
    train_set = results_train[:train_size,0:15], results_train[:train_size,15:976]
    valid_set = results_valid[:valid_size,0:15], results_valid[:valid_size,15:976]

    #####################
    train_size=int(len(results_train)*0.9/100)*100
    valid_size=int(len(results_train)*0.1/100)*100
    #note below 0:15 extracts 0-14 and 15:976 extracts 15-975
    train_set = results_train[:train_size,0:15], results_train[:train_size,15:976]
    valid_set = results_train[-valid_size:,0:15], results_train[-valid_size:,15:976]
    ########################

    train_mode = True
    mymodel = Model("saved_model2", train_mode=train_mode, input_dim=15, T=961,batch_size=256)
    if train_mode == True:
        mymodel.train(train_set, valid_set, maxEpoch=900)
...
def Retrain():
    ...
    mymodel = Model("saved_model2", train_mode=train_mode, input_dim=15, T=961,batch_size=256)
    if train_mode == True:
        mymodel.retrain(train_set, valid_set, maxEpoch=500)
...
# if __name__ == "__main__":
    ...
    path = 'bs2/carfentanil_1_120_0.1_bs2'
    files=glob.glob(path)
    with tf.Session() as sess:
        saver.restore(sess,"saved_model2/mymodel-5")
        for file in files:
            with open(file) as csvfile1:
                ...
                newfilename = file + '_predictedmirror_hamed4_3'
