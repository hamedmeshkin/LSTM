...
#                pdb.set_trace()
    ...
    def retrain(self, train_set, valid_set, maxEpoch=10):
         ...
         with tf.Session() as sess:
            ...
            saver = tf.train.Saver()
            saver.restore(sess, "saved_model/mymodel-1")
...
#                pdb.set_trace()
                ...
                self.save_model(saver, sess, step=epoch)
                np.savetxt('save_mirror/loss.txt', loss_v )
            self.file_writer.close()
...
# path = r'./bs1' # use your path
# all_files = glob.glob(os.path.join(path , "car*bs1"))
# mpg = []
...
# idx = np.random.choice(np.size(results_valid0,0),size=SIZE,replace=False)
...
if __name__ == "__main__":
    ...
    path = 'concat_bs2/concatenated_carfentanil_bs2'
    files=glob.glob(path) 
    with tf.Session() as sess:
        saver.restore(sess,"saved_model2/mymodel-89")
        for file in files:
            with open(file) as csvfile1:
                ...
                newfilename = 'concat_bs2/concatenated_predicted_carfentanil2_bs2'
